# ==================================== #
# Certificates bekessy.me US-EAST-1
# ==================================== #
...
# ==================================== #
# Certificates codeberryschool.com US-EAST-1
# ==================================== #
resource "aws_acm_certificate" "codeberryschool_com_us_east_1" {
  provider                  = aws.us_east_1
  domain_name               = "codeberryschool.com"
  subject_alternative_names = ["*.codeberryschool.com"]
  validation_method         = "DNS"
...
# DNS validation Route53 records US-EAST-1
resource "aws_route53_record" "codeberryschool_com_us_east_1_certificate_dns_validation_route" {
  provider = aws.us_east_1

  for_each = {
    for dvo in aws_acm_certificate.codeberryschool_com_us_east_1.domain_validation_options : dvo.domain_name => {
      name   = dvo.resource_record_name
      record = dvo.resource_record_value
      type   = dvo.resource_record_type
    }
...
# DNS validation objects US-EAST-1
resource "aws_acm_certificate_validation" "codeberryschool_com_us_east_1_certificate_dns_validation" {
  provider                = aws.us_east_1
  certificate_arn         = aws_acm_certificate.codeberryschool_com_us_east_1.arn
  validation_record_fqdns = [for record in aws_route53_record.codeberryschool_com_us_east_1_certificate_dns_validation_route : record.fqdn]
}
